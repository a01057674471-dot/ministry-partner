...
const quickTools = [
  ...
  ["이미지 콘텐츠", "카드뉴스와 썸네일 기획", "/image-content", "🎨"],
    ...
    <main className="mp-dashboard">
      ...
      <section className="mp-main">
        <div className="mp-content">
          ...
          <section className="mp-section"><h2>빠른 시작</h2><div className="mp-quick-grid">{quickTools.map(([title, desc, href, icon]) => <a href={href} key={title} className="mp-quick-card"><span>{icon}</span><strong>{title}</strong><small>{desc}</small></a>)}</div></section>
          <section className="mp-section"><div className="mp-section-title"><h2>이번 주 사역</h2><div>‹　›</div></div><div className="mp-ministry-grid">{ministry.map(([title, due, dday, progress, icon]) => <article key={String(title)}><div className="mp-ministry-head"><span>{icon}</span><div><strong>{title}</strong><small>{due}</small></div></div><b>{dday}</b><div className="mp-progress-label"><span>진행률 {progress}%</span><small>07</small></div><div className="mp-progress"><i style={{width: `${progress}%`}} /></div></article>)}</div></section>

...
export default function ResearchPage() {
  ...
  useEffect(() => {
    if (initialPassage) runResearch(initialPassage);
    // 최초 URL 본문만 자동 실행합니다.
    ...
    <main className="research-page">
      ...
      <section className="hero-card research-panel" style={{ marginTop: 34 }}>
        ...
        <form className="research-input" onSubmit={submit}>
          <input value={passage} onChange={(e) => setPassage(e.target.value)} placeholder="예: 요한복음 15장 1–8절" />
        ...
        <section className="result">
          ...
          <ResultSection title="원어 관찰">
            {result.originalLanguage.map((item, index) => (
              <p key={`${item.word}-${index}`}><strong>{item.word}</strong> ({item.transliteration}) — {item.meaning}</p>
            ))}
          </ResultSection>
...
function ListSection({ title, items }: { title: string; items: string[] }) {
  return <ResultSection title={title}><ul>{items.map((item, index) => <li key={index}>{item}</li>)}</ul></ResultSection>;
}

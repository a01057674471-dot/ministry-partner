...
type ToolType = "sermon" | "prayer" | "shorts" | "ideas" | "thumbnail" | "devotional" | "roadmap" | "document" | "meeting" | "youtube" | "file";
...
입력에 없는 숫자, 인원, 예산은 임의로 확정하지 말고 '협의 필요'라고 표시하세요. 추상적인 표현 뒤에는 반드시 누가, 언제, 무엇을, 어떻게 측정할지를 붙이세요.`,
  ...
  youtube: `사용자가 제공한 유튜브 주소와 함께 서버가 확보한 제목·설명·자막 등 실제 제공 정보를 분석하세요. 제공되지 않은 영상 장면이나 발언을 본 것처럼 꾸미지 마세요.
가능한 자료를 바탕으로 요즘 숏폼 형식에 맞는 후보 8개를 제안하세요. 각 후보마다 1) 제목 2) 선택 이유 3) 45~60초 재구성 대본 4) 첫 2초 훅 5) 장면 전환 6) 핵심 자막 5개 7) 썸네일 문구 3개 8) 인스타 본문 9) 고정댓글을 작성하세요. 자료가 제목과 설명뿐이라면 '메타데이터 기반 기획'임을 첫 줄에 분명히 표시하세요.`,
...
export async function POST(request: Request) {
  try {
    ...
    const metadata = tool === "youtube" ? await getYouTubeMetadata(topic) : "";
